...
from experiments import WHEAT_ALL_CLEAR_EXP, WHEAT_ALL_JUSTIFIED_EXP, DYNAMIC_WHEAT_CHECK, COCHLE_EXP
...
classes_prefs = ['control_day_', 'cochle_day_']
...
for day, ax in zip(days, axes):
    for class_pref, class_color in zip(classes_prefs, classes_colors):
        curr_curve = agg_curve(classes_features_dict[f'{class_pref}{day}'])
...
plt.savefig(f'{base_dir}/{exp_pref}_curves_{days_postf}.png', dpi=200)
plt.show()
...
for ax, index_name in zip(axes, indexes):
    for class_pref, class_color in zip(classes_prefs, classes_colors):
        day_indexes = [
            agg_index(classes_features_dict[f'{class_pref}{day}'], index_name=index_name)
            for day in days
        ]
...
plt.savefig(f'{base_dir}/{exp_pref}_indexes_by_days_{days_postf}.png')
plt.show()

index_df = pd.DataFrame(res_list)
index_df.to_csv(f'{base_dir}/{exp_pref}_indexes_by_days_{days_postf}.df', index=False)
